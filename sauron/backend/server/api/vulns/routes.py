...
vulns = Blueprint("auth", __name__, url_prefix="/api/v1/auth")


@vulns.route("/register", methods=["POST"])
def register():
    req = request.json
    if not req:
...
@vulns.route("/login", methods=["POST"])
def login():
    req = request.json
    if not req:
        raise BadRequestError("Fields cannot be empty.")
...
@vulns.route("/verify", methods=["POST"])
def verify():
...
@vulns.route("/logout", methods=["GET"])
def logout():
    clear_tokens()
